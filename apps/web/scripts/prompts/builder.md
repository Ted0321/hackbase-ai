...
- Public product summary copy must be usable on Prodia's project overview page. It fills THREE distinct roles that appear in three different places on the detail page — keep them clearly separated and DO NOT let their contents overlap:
  ...
  - `usageGuide` (role 4 — the 使い方 tab, rendered as NUMBERED step-by-step instructions): a top-level object `{ "intro"?, "steps": [{ "action", "result" }], "checkPoint"? }` with 2–4 steps that follow the actual demo flow (first render → primary action → what to read in the output). Each `action` is ONE imperative Japanese sentence telling the user exactly what to operate, naming the REAL UI element that exists in your `files[].content` — e.g. 「「サンプル実行トレースを再生」ボタンを押す」 — never a vague topic heading like 「入力と目的を確認する」. Each `result` is ONE Japanese sentence describing what visibly changes on screen after that action — e.g. 「結果エリアに各ステップの出力が順番に表示される」. Step 1's action must be performable on the default first render (align it with `interactionProofPlan.primaryAction`). `checkPoint` is one sentence telling the user WHERE on screen to look to judge the product's core value (typically where `mvpContract.inspectableOutput` appears). `intro` is an optional single framing sentence (when/why to use this). Keep each `action` ≤ ~50 chars and each `result` ≤ ~100 chars; no sentence may repeat `shortTagline` / `productSummary` / `interestingness`, and no two steps may share a sentence.
...
- REQUIRED: emit a non-empty top-level `usageGuide` object (2–4 steps, each with an imperative `action` naming a real UI element from your generated source and an on-screen `result`, plus a `checkPoint`), following the role-4 rules above. It renders verbatim as the 使い方 tab's numbered instructions, so every step must be something the user can actually do in the demo.
...
- Does every `usageGuide.steps[].action` name a control or element that actually exists in `files[].content`, is step 1's action clickable on the default first render, and do the steps avoid repeating any sentence from `shortTagline` / `productSummary` / each other?
...
Expected JSON shape:
{
  ...
  "usageGuide": {
    ...
    "steps": [
      { "action": "「サンプル実行トレースを再生」ボタンを押す", "result": "結果エリアに各ステップの出力が順番に表示される。" },
      { "action": "ステップごとの出力カードを読む", "result": "各処理が何を受け取り何を返したかが確認できる。" }
    ],
    "checkPoint": "one Japanese sentence: where on screen to look to judge the product's core value"
  },
