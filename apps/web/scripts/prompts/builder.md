...
- Public product summary copy must be usable on Prodia's project overview page. It fills THREE distinct roles that appear in three different places on the detail page — keep them clearly separated and DO NOT let their contents overlap:
  ...
  - `usageGuide` (role 4 — the 使い方 tab, rendered as NUMBERED steps): a top-level object `{ "intro"?, "steps": [{ "action", "result" }], "checkPoint"? }` with 2–5 steps structured as a DATA FLOW, not as UI operations: (1) input preparation — what data/material the user provides and how it enters the product, grounded in `source/data/sample-input.ts`; (2) handling notes — concrete caveats about what to include/exclude, ONLY when such constraints actually exist in your generated source (omit this step entirely rather than inventing caveats); (3) what the AI does — the role, criteria/viewpoints, and output schema taken from the ACTUAL prompt template in your `source/core/steps/*.ts`, e.g. 「安全分析者の役割を与え、資料を横断して危険源と作業前チェック項目を構造化データで抽出させる設計です」; (4) output — the concrete information that appears, quoting ONE real example from `source/data/sample-trace.ts`, e.g. 「高圧ガス配管(A-17)」. Each `action` is a short Japanese stage heading (≤ ~50 chars, no trailing 。), e.g. 「設備IDを起点に資料を集める」. Each `result` is 2–3 Japanese sentences (≤ ~200 chars total) following the pattern: what happens / by what criteria or viewpoint / a sample-trace example. FORBIDDEN: screen-layout vocabulary (画面中央・右側・上部・「〜エリア」の位置説明 — naming a button or panel by its LABEL is fine); claiming the demo performs live AI execution — describe processing as the design (「〜する設計です」) and state once, in `checkPoint` or the final step, that the demo replays a recorded sample trace. `checkPoint` is 1–2 sentences telling the user what to look at to judge the core value. `intro` is an optional single framing sentence. Every factual claim must be traceable to your own `files[].content`; no sentence may repeat `shortTagline` / `productSummary` / `interestingness`, and no two steps may share a sentence.
...
- Put rich Japanese product copy, source provenance, readiness, and risk detail in the top-level BuildPlan fields (`shortTagline`, `productSummary`, `interestingness`, `usageGuide`, `sourceTrace`, `visualIdentity`, `submissionReadiness`, `mvpContract`, `mvpContractV2`, `implementationNotes`, `knownRisks`) instead of duplicating it inside `metadata.json` file content.
...
- REQUIRED: emit a non-empty top-level `usageGuide` object (2–5 data-flow steps — input preparation → handling notes (only when real) → what the AI does → concrete output — each `result` 2–3 sentences grounded in your `files[].content`, plus a `checkPoint`), following the role-4 rules above. It renders verbatim as the 使い方 tab's numbered instructions; no screen-layout vocabulary, and never claim live AI execution (state once that the demo replays a recorded trace).
...
- Every `files[].content` that materializes to a `*.json` file must be EXACTLY one JSON document: end the content at the final closing `}` with nothing after it — no trailing notes, comments, duplicate fragments, or stray characters. Even one leftover character after the closing brace makes the strict gate fail the whole build (a real `validation/self-review.json` build was lost this way).
...
- Is top-level `framework` exactly `next_static_artifact`, and are ALL top-level fields present (`shortTagline`, `productSummary`, `interestingness`, `usageGuide`, `categoryId`, `categoryReason`, `sourceTrace`, `submissionReadiness`, `mvpContract`, `mvpContractV2`, `interactionModel`, `interactionProofPlan`)?
...
- Does `usageGuide` read as a data flow (input preparation → what the AI does → concrete output) with every factual claim traceable to `files[].content` (prompt templates in `source/core/steps/*.ts`, examples from `source/data/sample-trace.ts`), with NO screen-layout vocabulary (画面中央/右側/上部 etc.), no live-execution claims (the trace-replay boundary stated once), and no sentence repeated from `shortTagline` / `productSummary` / other steps?
...
Expected JSON shape:
{
  ...
  "usageGuide": {
    "intro": "optional: one Japanese sentence framing when/why to use this product",
    "steps": [
      { "action": "設備IDを起点に資料を集める", "result": "入力は設備ID(サンプルでは PLT-EAST-03A)。このIDに紐づく点検マニュアルや過去の作業記録を、AIに渡せる形にまとめます。" },
      { "action": "AIが安全分析者として資料を読み解く", "result": "プロンプトで安全分析者の役割を与え、資料を横断して危険源と作業前チェック項目を抽出させる設計です。出力は「危険源(名前+説明)」と「チェック項目(重要度フラグ付き)」の構造化データに固定してあります。" },
      { "action": "図面付きブリーフィングが組み上がる", "result": "サンプルでは「高圧ガス配管(A-17)」が図面上の位置付きで抽出され、「バルブA-17の締付トルクを確認」など重要度付きのチェック項目が並びます。" }
    ],
    "checkPoint": "デモは記録済みトレースの再生です。チェック項目がそのまま実行できる粒度になっているかを見てください。"
  },
